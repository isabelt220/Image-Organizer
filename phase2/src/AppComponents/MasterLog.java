...
public class MasterLog implements Serializable {
  ...
  /**
   * Ordered log that will keep track of all tag and image modifications Maps key timestamp to value
   * description.
   */
   ...
   *
   * @param currentTagNames String the tags the image contains (could be empty) when it is first
   *     imported
  ...
  /**
   * Called to add log entry with appropriate description when an image was un-associated with the
   * tag.
   *
   * @param tagName String name of tag that was added to the image
   * @param imageName String path of the image file
  ...
  /**
   * Public getter for master log. Used to display master log.
   *
   * @return this log
   */
