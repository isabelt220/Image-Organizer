...
/**
 * Helper class for ImageData and is therefore always initialized by and attached to a ImageData, stores information
 * regarding all the modifications of tags of the ImageData, and can be used to revert back to tags of a certain time point
 */
public class ImageLog implements Serializable{

    //nameLog is a ordered hashmap with a timestamp of each tag modification mapped to the image name at that timestamp.
    private LinkedHashMap<String, String> nameLog;
    //tagLog is a ordered hashmap with a timestamp of each tag modification mapped to the array list of associated tag names prior to the tag modification.
    private LinkedHashMap<String, ArrayList<String>> tagLog;
//    //for testing purposes only
//    private String lastChangeTime;

    /**
     * Constructor of a new ImageLog object, is always called upon by an ImageData object.
     * Initializes the two logs, and adds the first entry recording the name, whether if the image file comes with pre-existing tags, and their
     * specific tag names.
     *
    ...
    /**
     * Called upon whenever there is a new tag modification to the ImageData,
     * takes the old and new tagList of the ImageData before and after the tag modification, calls a helper method to
     * convert them to an ArrayLists of tagNames, and adds them to the two logs in the specified format.
     *
     ...
     */
    public void addEntry(String location, String coreName, ArrayList<Tag> newTagArrayList, ArrayList<Tag>oldTagArrayList){
        Timestamp time = new Timestamp(System.currentTimeMillis());
        ...
        MainContainer.getMasterLog().addEntry(time.toString(), "Image: " + location+" Tags: ["+oldName+"] --> ["+newName+"]");
    }
    ...
    /**
     * Checks if the other is an ImageLog with the same nameLog as this ImageLog, nameLogs should be unique.
